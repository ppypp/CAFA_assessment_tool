...
class GOPrediction:
    ...
    def read_and_split_and_write(self, obo_path, prediction_path):
        ...
        mfo_out = open("{}_MFO.txt".format(os.path.splitext(prediction_path.name)[0]), "w")
        bpo_out = open("{}_BPO.txt".format(os.path.splitext(prediction_path.name)[0]), "w")
        cco_out = open("{}_CCO.txt".format(os.path.splitext(prediction_path.name)[0]), "w")
